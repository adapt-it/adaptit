...
// GDLC 17FEB16
// Changed file extension from .c to .cpp and enclosed code in extern "C" because the compiler
// in Xcode 7 needed that.
// Also changed three local identifiers because the Xcode 7 compiler needed that:-
// class -> classx
// new -> newx
// _tvdiff -> xtvdiff
...
struct mdnsd_struct
{
    ...
    int classx, frame;  // GDLC 17FEB16 was int class, frame;
...
int xtvdiff(struct timeval old, struct timeval newx)    // GDLC 17FEB16 Was int _tvdiff(struct timeval old, struct timeval new)
{
    int udiff = 0;
    if(old.tv_sec != newx.tv_sec) udiff = (newx.tv_sec - old.tv_sec) * 1000000;
    return (newx.tv_usec - old.tv_usec) + udiff;
}
...
int _r_out(mdnsd d, struct message *m, mdnsdr *list) // BEW d->classx, see mdnsd_struct definition line 106, has int classx,frame;
...
{ // copy a published record into an outgoing message
    ...
    while((r = *list) != 0 && message_packet_len(m) + _rr_len(&r->rr) < d->frame)
    {
        *list = r->list;
        ret++;
        if(r->unique)
            message_an(m, r->rr.name, r->rr.type, (unsigned short)d->classx + 32768, r->rr.ttl);
        else
			message_an(m, r->rr.name, r->rr.type, (unsigned short)d->classx, r->rr.ttl);
        _a_copy(m, &r->rr);
...
mdnsd mdnsd_new(int classx, int frame)
{
...
void mdnsd_in(mdnsd d, struct message *m, unsigned long int ip, unsigned short int port) // BEW added cast (char*) & (const char*)
{
    ...
    if(m->header.qr == 0)
    {
        ...
        { // process each query
            if(m->qd[i].rr_class != d->classx || (r = _r_next(d,0,(char*)m->qd[i].name,m->qd[i].type)) == 0) continue;
    ...
    *ip = inet_addr("224.0.0.251");
    ...
    { // send out individual unicast answers
        ...
        *ip = u->to;
        ...
        message_qd(m, u->r->rr.name, u->r->rr.type, (unsigned short)d->classx);
		message_an(m, u->r->rr.name, u->r->rr.type, (unsigned short)d->classx, u->r->rr.ttl);
        _a_copy(m, &u->r->rr);
        free(u);
        return 1;
    }
...
//printf("OUT: probing %X now %X pause %X publish %X\n",d->probing,d->a_now,d->a_pause,d->a_publish);
    ...
    if(d->a_publish && xtvdiff(d->now,d->publish) <= 0)
    { // check to see if it's time to send the publish retries (and unlink if done)
        ...
        while(cur && message_packet_len(m) + _rr_len(&cur->rr) < d->frame)
        {
            ...
            ret++; cur->tries++;
            if(cur->unique)
				message_an(m, cur->rr.name, cur->rr.type, (unsigned short)d->classx + 32768, cur->rr.ttl);
            else
				message_an(m, cur->rr.name, cur->rr.type, (unsigned short)d->classx, cur->rr.ttl);
            _a_copy(m, &cur->rr);
    ...
    if(d->a_pause && xtvdiff(d->now, d->pause) <= 0) ret += _r_out(d, m, &d->a_pause);
    ...
    if(d->probing && xtvdiff(d->now,d->probe) <= 0)
    {
        mdnsdr last = 0;
        for(r = d->probing; r != 0;)
        { // scan probe list to ask questions and process published
			...
			message_qd(m, r->rr.name, r->rr.type, (unsigned short)d->classx);
            last = r;
            r = r->list;
        }
        ...
        { // scan probe list again to append our to-be answers
            r->unique++;
			message_ns(m, r->rr.name, r->rr.type, (unsigned short)d->classx, r->rr.ttl);
            _a_copy(m, &r->rr);
            ret++;
        }
        if(ret)
    ...
    { // process qlist for retries or expirations
        ...
        for(q = d->qlist; q != 0; q = q->list)
            if(q->nexttry > 0 && q->nexttry <= (unsigned long)d->now.tv_sec && q->tries < 3)
                message_qd(m,(unsigned char*)q->name,(unsigned short)q->type,(unsigned short)d->classx);
        ...
        for(q = d->qlist; q != 0; q = q->list)
        {
            ...
            {
                message_an(m,(unsigned char*)q->name,(unsigned short)q->type,(unsigned short)d->classx,c->rr.ttl - d->now.tv_sec);
                _a_copy(m,&c->rr);
            }
        }
...
struct timeval *mdnsd_sleep(mdnsd d)
{
    ...
    { // then check for paused answers
        if((usec = xtvdiff(d->now,d->pause)) > 0) d->sleep.tv_usec = usec;
        RET;
    }

    if(d->probing)
    { // now check for probe retries
        if((usec = xtvdiff(d->now,d->probe)) > 0) d->sleep.tv_usec = usec;
        RET;
    }

    if(d->a_publish)
    { // now check for publish retries
        if((usec = xtvdiff(d->now,d->publish)) > 0) d->sleep.tv_usec = usec;
        RET;
    }
