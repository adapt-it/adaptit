...
bool wxDialogHelper::CanDoLayoutAdaptation()
{
    ...
    return (layoutEnabled && !GetLayoutAdaptationDone() && GetLayoutAdaptationLevel() != 0 && GetLayoutAdapter() != NULL && GetLayoutAdapter()->CanDoLayoutAdaptation(this));
}
...
bool wxStandardDialogLayoutAdapter::DoLayoutAdaptation(wxDialogHelper* dialog)
{
    if (dialog->GetDialog()->GetSizer())
    {
        ...
        else
        {
            ...
            wxScrolledWindow* scrolledWindow = CreateScrolledWindow(dialog->GetDialog());
