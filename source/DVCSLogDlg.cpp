...
{        
	wxListCtrl*     pList = (wxListCtrl*) FindWindowById (ID_LST_VERSIONS);
...
// Add the columns -- author, date, comment
    ...
    col1.SetWidth(300);
    pList->InsertColumn(1, col1);
    
    col2.SetId(2);
    col2.SetText( _T("Comment") );
    col2.SetWidth(600);
    pList->InsertColumn(2, col2);
    ...
    {        
        ...
        pList->SetItem (n, 0, name);                // put in first column
    ...
    // now set the column contents:
        pList->SetItem (n, 0, name);
        pList->SetItem (n, 1, date);
        pList->SetItem (n, 2, comment);
