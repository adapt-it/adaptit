...
/// functions used by Adapt It. These resources were created initially by wxDesigner. 
/// The wxDesigner generated C++ code in Adapt_It_wdr.cpp is #include into this implementation file. 
/// The Adapt_It_wdr.h and Adapt_It_wdr.cpp files are still incorporated into the Adapt It project
/// but require the running of the sedConvert.sh script to make them compatible with the wxWidgets
/// 3.1.x library (the script updates depricated font symbols and removes some wxALIGN... flags
/// to prevent run-time asserts from the 3.1.x library from pestering the user. 
