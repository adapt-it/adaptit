...
/// The KBSharingAuthenticationDlg class provides a dialog for turning on KB sharing, in a
...
void KBSharingAuthenticationDlg::OnOK(wxCommandEvent& myevent)
{
	...
	// as got from KBSharingAuthenticationDlg should be sought from m_strStatelessUsername.
	...
	if (m_bUserIsAuthenticating)
	{
		if (!pwd.IsEmpty())
		{
			...
			if (!bUserIsValid)
			{
				...
				m_pApp->LogUserAction(_T("In OnOK() of KBSharingAuthenticationDlg.cpp, password not recognised by KBserver"));
	...
	else
	{
		...
		if (!pwd.IsEmpty())
		{
			...
			if (!bUserIsValid)
			{
				...
				m_pApp->LogUserAction(_T("In OnOK() of KBSharingAuthenticationDlg.cpp, username or password not recognised by KBserver"));
				wxString msg = _("Most likely your chosen KBserver is not yet running. Check.\nOr maybe the username ( %s ) is not in the list of users for this knowledge base server.\nOr the URL, or password, is not correct.\nPerhaps ask your server administrator to help you.\nYou can continue working, but KB sharing will be OFF");
				msg = msg.Format(msg, m_strStatelessUsername.c_str());
				wxMessageBox(msg, _("Authentication: a check failed"), wxICON_WARNING | wxOK);
				//this->Show(TRUE); // make the dialog visible again, we aren't done with it yet
...
#if defined(_DEBUG)
		wxLogDebug(_T("Authenticate Dlg (KBSharingAuthenticationDlg.cpp) URL: %s , stored in m_pApp->m_strKbSErverURL"),
...
#if defined(_DEBUG)
wxLogDebug(_T("Authenticate Dlg (KBSharingAuthenticationDlg.cpp) URL: %s , stored in m_pApp->m_strStatelessURL"),
		   m_pApp->m_strStatelessURL.c_str());
#endif
	}
