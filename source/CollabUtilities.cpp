...
// collaboration project or there was some other error.
...
{
	CAdapt_ItApp* pApp = &wxGetApp();
	if (!(pSetupDlg->m_bTempCollaboratingWithParatext || pSetupDlg->m_bTempCollaboratingWithBibledit))
	{
		// Return FALSE because this currently open project has not been setup to be a collaboration project
		return FALSE;
	}
	wxASSERT(!pApp->m_collaborationEditor.IsEmpty()); // there must be a collaborating software editor (BibleEdit or ParaTExt)
	...
	// been effected before AdjustForCollaborationTypeChange() is called. If the AI project is concurrently
	// changed, it's quite possible that the flag value passed in may be already correct, so we must
	// take that possibility into account further below - if so, we'd not need to split or join
	...
	if (!bOK)
	{
		wxString msg = _T("AdjustForCollaborationTypeChange(): the EnumerateDocFiles_ParametizedStore() function failed");
		pApp->LogUserAction(msg);
		return FALSE;
	}
#if defined(_DEBUG) && defined(TYPE_CHANGED)
	{
		...
		for (index = 0; index < count; index++)
		{
			...
			wxLogDebug(_T("AdjustForCollaborationTypeChange() allDocsArr: doc filename = %s"), name.c_str());
		}
	}
#endif
	...
	if (bCurrentCollabByChapterOnly)
	{
		...
		wxASSERT(pApp->m_bCollabByChapterOnly == FALSE); // the app's setting, which is not yet reflecting
			// the user's in-dialog choice, should still be the "whole book" setting, as starting point
	...
	else
	{
		...
		wxASSERT(pApp->m_bCollabByChapterOnly == TRUE); // the app's setting, which is not yet reflecting
			// the user's in-dialog choice, should still be the "by chapter only" setting, as starting point
