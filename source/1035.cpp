...
// Also changed the macro my(x,y) because Xcode 7 complains about mismatch between the
// void pointer in my(x,y) and the struct members to which the macro expansions assign.
...
void message_parse(struct message *m, unsigned char *packet)
{
    ...
    // GDLC 17FEB16 Xcode 7 complains about mismatch between the void pointer in my(x,y) and
    // the struct members to which the macro expansions assign. So a macro is needed for each type.
    //#define my(x,y) while(m->_len&7) m->_len++; x = (void*)(m->_packet + m->_len); m->_len += y;
    #define myq(x,y) while(m->_len&7) m->_len++; x = (question*)(m->_packet + m->_len); m->_len += y;
    #define myr(x,y) while(m->_len&7) m->_len++; x = (resource*)(m->_packet + m->_len); m->_len += y;
    ...
    // process questions
    // GDLC 17FEB16 Xcode 7 complains about mismatch between the void pointer in my(x,y) and
    // the struct members to which the macro expansions assign. The following expansion was found to compile.
    //while (m->_len&7)
    //{
    //    m->_len++;
    //    m->qd = (question*)(m->_packet + m->_len);
    //    m->_len += sizeof(struct question) * m->qdcount;
    //}
    myq(m->qd, sizeof(struct question) * m->qdcount);
    ...
    // process rrs
    myr(m->an, sizeof(struct resource) * m->ancount);
    myr(m->ns, sizeof(struct resource) * m->nscount);
    myr(m->ar, sizeof(struct resource) * m->arcount);
...
void message_qd(struct message *m, unsigned char *name, unsigned short int type, unsigned short int classx)
{
    m->qdcount++;
    if(m->_buf == 0) m->_buf = m->_packet + 12; // initialization
    _host(m, &(m->_buf), name);
    short2net(type, &(m->_buf));
    short2net(classx, &(m->_buf));
}

void _rrappend(struct message *m, unsigned char *name, unsigned short int type, unsigned short int classx, unsigned long int ttl)
{
    if(m->_buf == 0) m->_buf = m->_packet + 12; // initialization
    _host(m, &(m->_buf), name);
    short2net(type, &(m->_buf));
...
void message_an(struct message *m, unsigned char *name, unsigned short int type, unsigned short int classx, unsigned long int ttl)
{
    m->ancount++;
    _rrappend(m,name,type,classx,ttl);
}

void message_ns(struct message *m, unsigned char *name, unsigned short int type, unsigned short int classx, unsigned long int ttl)
{
    m->nscount++;
    _rrappend(m,name,type,classx,ttl);
}

void message_ar(struct message *m, unsigned char *name, unsigned short int type, unsigned short int classx, unsigned long int ttl)
{
    m->arcount++;
    _rrappend(m,name,type,classx,ttl);
}
