...
void CConflictResActionDlg::InitDialog(wxInitDialogEvent& WXUNUSED(event))
{
	...
	wxString middleTextBoxStr = _("This dialog opens at the first verse conflict encountered. A verse conflict means that the Adapt It form of the verse differs from the same verse in %s. If you choose this radio button, no additional dialog appears. Instead, every time Adapt It finds a verse conflict, it will automatically choose the Adapt It form of the verse, and transfer that to %s. This will permanently erase the content of that verse in %s, replacing it with what was transferred from Adapt It. (If you need more control, choose the button below.) ");
	wxString bottomTextBoxStr = _("For this Save operation, all the identified verse conflicts will be shown to you in a dialog which appears just once. The conflicting verses are listed. Clicking a verse reference in the list, shows the Adapt It and %s versions of that verse, and you can choose the best version. (No editing allowed.) The versions are displayed side by side; the source text is above. Make your choice either using radio buttons, or by clicking a checkbox to cause the Adapt It form of the verse to be chosen and transferred.");
