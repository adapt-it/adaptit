...
static void init_char_count(char_count_t char_cnt[])
{
    for (size_t i = 0; i < MAX_CHAR; ++i) {
        char_cnt[i].first = (uint16_t)i; // whm modified 12Jun09 added (uint16_t) cast
        char_cnt[i].second = 0;
    }
}
...
const char* tellenc2(const unsigned char* const buffer, const size_t len)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        if (last_ch != EOF) {
            uint16_t dbyte_char = (uint16_t)((last_ch << 8) + ch); // whm modified 12Jun09 added (unit16_t) cast
