...
/// \param  nStartAt        -> index in arr of the CSourcePhrase at which we start composing
///                            the oldSrcKeysStr containing the unique words in the span
...
/////////////////////////////////////////////////////////////////////////////////////////////
int GetUniqueSrcKeysAsAString(SPArray& arr, int nStartAt, wxString& oldSrcKeysStr, int limit)
{
...
// Like GetUniqueSrcKeysAsAString() except removing duplicates is not done, the m_key words
...
int GetKeysAsAString_KeepDuplicates(SPArray& arr, int nStartAt, wxString& keysStr, int limit)
{
	int count = arr.GetCount();
	int activeSpanCount = count - nStartAt;
	keysStr.Empty();
...
/// \param  nStartAt        -> index for the CSourcePhrase instance in arr from which we
///                            commence looking for words in common
...
////////////////////////////////////////////////////////////////////////////////////////
int GetWordsInCommon(SPArray& arr, int nStartAt, wxString& uniqueKeysStr, wxArrayString& strArray, int limit)
{
	strArray.Clear();
	int count = arr.GetCount();
	int activeSpanCount = count - nStartAt;
					 ...
					 int newStartAt, wxArrayString& strArray, int limit)
{
	...
	int oldUniqueWordsCount = GetUniqueSrcKeysAsAString(arrOld, oldStartAt, oldUniqueSrcWords, limit);
#ifdef __WXDEBUG__
	...
	int commonsCount = GetWordsInCommon(arrNew, newStartAt, oldUniqueSrcWords, strArray, limit);
#ifdef __WXDEBUG__
	{
		int i;
		wxLogDebug(_T("  commonsCount = %d"), commonsCount);
		for (i = 0; i < commonsCount; i++)
		{
			wxString word = strArray.Item(i);
			wxLogDebug(_T("Common word:   %s"), word.c_str());
		}
	}
#endif
	return commonsCount;
...
void MergeUpdatedSourceText(SPList& oldList, SPList& newList, SPList* pMergedList, int limit)
{
	...
	tuple[2] = pSubspan;
	// initialize tuple[2]
	pSubspan->childSubspans[0] = NULL; pSubspan->childSubspans[1] = NULL; pSubspan->childSubspans[2] = NULL; 
	pSubspan->oldStartPos = 0;
	pSubspan->newStartPos = 0;
	pSubspan->oldEndPos = oldSPCount - 1;
	pSubspan->newEndPos = newSPCount - 1;
	pSubspan->spanType = afterSpan;
							   ...
							   int limit, Subspan* tuple[])
{
	...
	for (tupleIndex = 0; tupleIndex < siz; tupleIndex++)
	{
		...
		else
		{
			...
			// allOldSrcWords
			int oldWordCount = 0;
			oldWordCount = GetKeysAsAString_KeepDuplicates(arrOld, oldStartAt, allOldSrcWords, limit); 													
			...
			// allNewSrcWords
			int newWordCount = 0;
			newWordCount = GetKeysAsAString_KeepDuplicates(arrNew, newStartAt, allNewSrcWords, limit); 													
			...
			int commonsCount = GetWordsInCommon(arrOld, oldStartAt, arrNew, newStartAt, arrInCommon, limit);
		...
		} // end of else block for test: if (tupleIndex == 1)
