//------------------------------------------------------------------------------
// Header generated by wxDesigner from file: Adapt_It.wdr
// Do not modify this file, all changes will be lost!
//------------------------------------------------------------------------------

#ifndef __WDR_Adapt_It_H__
#define __WDR_Adapt_It_H__

#if defined(__GNUG__) && !defined(NO_GCC_PRAGMA)
    #pragma interface "Adapt_It_wdr.h"
#endif

// Include wxWidgets' headers

#ifndef WX_PRECOMP
    #include <wx/wx.h>
#endif

#include <wx/image.h>
#include <wx/statline.h>
#include <wx/spinbutt.h>
#include <wx/spinctrl.h>
#include <wx/splitter.h>
#include <wx/listctrl.h>
#include <wx/treectrl.h>
#include <wx/notebook.h>
#include <wx/grid.h>
#include <wx/toolbar.h>
#include <wx/tglbtn.h>

// Declare window functions

const int ID_STATICBITMAP = -1;
const int ID_TEXT = 0;
const int ID_ABOUT_VERSION_LABEL = 1;
const int ID_ABOUT_VERSION_NUM = 2;
const int ID_ABOUT_VERSION_DATE = 3;
const int ID_STATIC_UNICODE_OR_ANSI = 4;
const int ID_STATIC_WX_VERSION_USED = 5;
const int ID_LINE = 6;
const int ID_STATIC_UI_LANGUAGE = 7;
const int ID_STATIC_HOST_OS = 8;
const int ID_STATIC_SYS_LANGUAGE = 9;
const int ID_STATIC_SYS_LOCALE_NAME = 10;
const int ID_STATIC_CANONICAL_LOCALE_NAME = 11;
const int ID_STATIC_SYS_ENCODING_NAME = 12;
const int ID_STATIC_SYSTEM_LAYOUT_DIR = 13;
wxSizer *AboutDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

extern wxSizer *ID_CONTROLBAR_1_LINE_SIZER;
const int IDC_RADIO_DRAFTING = 14;
const int IDC_RADIO_REVIEWING = 15;
const int IDC_CHECK_SINGLE_STEP = 16;
const int IDC_CHECK_KB_SAVE = 17;
const int IDC_CHECK_FORCE_ASK = 18;
const int IDC_BUTTON_NO_ADAPT = 19;
const int IDC_STATIC = 20;
const int IDC_EDIT_DELAY = 21;
const int IDC_CHECK_ISGLOSSING = 22;
wxSizer *ControlBarFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_COMPOSE = 23;
const int IDC_BUTTON_SHORTEN = 24;
const int IDC_BUTTON_PREV = 25;
const int IDC_BUTTON_LENGTHEN = 26;
const int IDC_BUTTON_NEXT = 27;
const int IDC_BUTTON_REMOVE = 28;
const int IDC_BUTTON_APPLY = 29;
const int IDC_STATIC_SECTION_DEF = 30;
const int IDC_RADIO_PUNCT_SECTION = 31;
const int IDC_RADIO_VERSE_SECTION = 32;
const int IDC_BUTTON_CLEAR = 33;
const int IDC_BUTTON_SELECT_ALL = 34;
wxSizer *ComposeBarFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_LISTBOX_ADAPTIONS = 35;
wxSizer *OpenExistingProjectDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_FILENAME = 36;
const int ID_TEXT_INVALID_CHARACTERS = 37;
wxSizer *GetOutputFilenameDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_SOURCE_LANGUAGE = 38;
const int ID_EDIT_SOURCE_LANG_CODE = 39;
const int IDC_TARGET_LANGUAGE = 40;
const int ID_EDIT_TARGET_LANG_CODE = 41;
const int ID_BUTTON_LOOKUP_CODES = 42;
wxSizer *LanguagesPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_SOURCE_FONTNAME = 43;
const int IDC_CHECK_SRC_RTL = 44;
const int IDC_SOURCE_SIZE = 45;
const int IDC_SOURCE_LANG = 46;
const int IDC_BUTTON_SOURCE_COLOR = 47;
const int ID_BUTTON_CHANGE_SRC_ENCODING = 48;
const int IDC_TARGET_FONTNAME = 49;
const int IDC_CHECK_TGT_RTL = 50;
const int IDC_TARGET_SIZE = 51;
const int IDC_TARGET_LANG = 52;
const int IDC_BUTTON_TARGET_COLOR = 53;
const int ID_BUTTON_CHANGE_TGT_ENCODING = 54;
const int IDC_NAVTEXT_FONTNAME = 55;
const int IDC_CHECK_NAVTEXT_RTL = 56;
const int IDC_NAVTEXT_SIZE = 57;
const int IDC_CHANGE_NAV_TEXT = 58;
const int IDC_BUTTON_NAV_TEXT_COLOR = 59;
const int ID_BUTTON_CHANGE_NAV_ENCODING = 60;
const int ID_TEXTCTRL_AS_STATIC_FONTPAGE = 61;
const int IDC_BUTTON_SPECTEXTCOLOR = 62;
const int IDC_RETRANSLATION_BUTTON = 63;
wxSizer *FontsPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_PLEASE_WAIT = 64;
wxSizer *WaitDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_LIST_ACCEPTED = 65;
const int IDC_BUTTON_REJECT = 66;
const int IDC_BUTTON_ACCEPT = 67;
const int ID_BUTTON_REJECT_ALL_FILES = 68;
const int ID_BUTTON_ACCEPT_ALL_FILES = 69;
const int IDC_LIST_REJECTED = 70;
wxSizer *WhichFilesDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_TRANSFORM_TO_GLOSSES1 = 71;
const int ID_TEXTCTRL_TRANSFORM_TO_GLOSSES2 = 72;
const int ID_TEXTCTRL_TRANSFORM_TO_GLOSSES3 = 73;
wxSizer *TransformToGlossesDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_CHECK_SOURCE_USES_CAPS = 74;
const int ID_CHECK_USE_AUTO_CAPS = 75;
const int ID_TEXTCTRL_AS_CASE_PAGE_STATIC_TEXT = 76;
const int ID_TEXT_SL = 77;
const int ID_TEXT_TL = 78;
const int ID_TEXT_GL = 79;
const int IDC_EDIT_SRC_CASE_EQUIVALENCES = 80;
const int IDC_EDIT_TGT_CASE_EQUIVALENCES = 81;
const int IDC_EDIT_GLOSS_CASE_EQUIVALENCES = 82;
const int IDC_BUTTON_CLEAR_SRC_LIST = 83;
const int IDC_BUTTON_CLEAR_TGT_LIST = 84;
const int IDC_BUTTON_CLEAR_GLOSS_LIST = 85;
const int IDC_BUTTON_SRC_SET_ENGLISH = 86;
const int IDC_BUTTON_TGT_SET_ENGLISH = 87;
const int IDC_BUTTON_GLOSS_SET_ENGLISH = 88;
const int IDC_BUTTON_SRC_COPY_TO_NEXT = 89;
const int IDC_BUTTON_TGT_COPY_TO_NEXT = 90;
const int IDC_BUTTON_GLOSS_COPY_TO_NEXT = 91;
const int IDC_BUTTON_SRC_COPY_TO_GLOSS = 92;
wxSizer *CaseEquivDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_WELCOME_TO = 93;
const int IDC_EDIT_ADAPT_IT = 94;
const int ID_TEXTCTRL_AS_STATIC_WELCOME = 95;
const int IDC_CHECK_NOLONGER_SHOW = 96;
wxSizer *WelcomeDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_AS_STATIC_DOCPAGE = 97;
const int IDC_STATIC_WHICH_MODE = 98;
const int IDC_BUTTON_WHAT_IS_DOC = 99;
const int IDC_STATIC_WHICH_FOLDER = 100;
const int IDC_LIST_NEWDOC_AND_EXISTINGDOC = 101;
const int IDC_CHECK_CHANGE_FIXED_SPACES_TO_REGULAR_SPACES = 102;
const int IDC_CHANGE_FOLDER_BTN = 103;
const int IDC_CHECK_FORCE_UTF8 = 104;
wxSizer *DocPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_AS_STATIC_PROJECTPAGE = 105;
const int IDC_BUTTON_WHAT_IS_PROJECT = 106;
const int IDC_LIST_NEW_AND_EXISTING = 107;
wxSizer *ProjectPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_SRC = 108;
const int IDC_LIST_PUNCTS = 109;
const int ID_TEXTCTRL_AS_STATIC_PLACE_INT_PUNCT = 110;
const int IDC_EDIT_TGT = 111;
const int IDC_BUTTON_PLACE = 112;
wxSizer *PlaceInternalPunctDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_RADIO_INCHES = 113;
const int IDC_RADIO_CM = 114;
wxSizer *UnitsDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_MATCHED_SOURCE = 115;
const int IDC_MYLISTBOX_TRANSLATIONS = 116;
const int IDC_EDIT_REFERENCES = 117;
const int IDC_BUTTON_MOVE_UP = 118;
const int IDC_BUTTON_MOVE_DOWN = 119;
const int IDC_BUTTON_CANCEL_ASK = 120;
const int IDC_BUTTON_CANCEL_AND_SELECT = 121;
const int IDC_EDIT_NEW_TRANSLATION = 122;
wxSizer *ChooseTranslationDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_STATIC_SRC = 123;
const int IDC_EDIT_PRECONTEXT = 124;
const int IDC_EDIT_SOURCE_TEXT = 125;
const int IDC_EDIT_RETRANSLATION = 126;
const int IDC_COPY_RETRANSLATION_TO_CLIPBOARD = 127;
const int IDC_BUTTON_TOGGLE = 128;
const int IDC_STATIC_TGT = 129;
const int IDC_EDIT_FOLLCONTEXT = 130;
wxSizer *RetranslationDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_SRC_TEXT = 131;
const int IDC_EDIT_TGT_TEXT = 132;
const int IDC_SPIN_CHAPTER = 133;
const int IDC_SPIN_VERSE = 134;
const int IDC_GET_CHVERSE_TEXT = 135;
const int IDC_CLOSE_AND_JUMP = 136;
const int IDC_SHOW_MORE = 137;
const int IDC_SHOW_LESS = 138;
const int IDC_STATIC_BEGIN = 139;
const int IDC_STATIC_END = 140;
wxSizer *EarlierTransDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_CHECK_KB_BACKUP = 141;
const int IDC_CHECK_BAKUP_DOC = 142;
const int ID_TEXTCTRL_AS_STATIC_BACKUPS_AND_KB_PAGE = 143;
const int IDC_EDIT_SRC_NAME = 144;
const int IDC_EDIT_TGT_NAME = 145;
const int IDC_RADIO_ADAPT_BEFORE_GLOSS = 146;
const int IDC_RADIO_GLOSS_BEFORE_ADAPT = 147;
const int IDC_CHECK_LEGACY_SRC_TEXT_COPY = 148;
wxSizer *BackupsAndKBPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int STATIC_TEXT_V4 = 149;
const int IDC_EDIT_LEADING = 150;
const int STATIC_TEXT_V5 = 151;
const int IDC_EDIT_GAP_WIDTH = 152;
const int STATIC_TEXT_V6 = 153;
const int IDC_EDIT_LEFTMARGIN = 154;
const int STATIC_TEXT_V7 = 155;
const int IDC_EDIT_MULTIPLIER = 156;
const int IDC_EDIT_DIALOGFONTSIZE = 157;
const int IDC_CHECK_WELCOME_VISIBLE = 158;
const int IDC_CHECK_HIGHLIGHT_AUTO_INSERTED_TRANSLATIONS = 159;
const int ID_PANEL_AUTO_INSERT_COLOR = 160;
const int IDC_BUTTON_CHOOSE_HIGHLIGHT_COLOR = 161;
const int IDC_CHECK_SHOW_ADMIN_MENU = 162;
wxSizer *ViewPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

wxSizer *UnitsPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_AS_STATIC_AUTOSAVE = 163;
const int IDC_CHECK_NO_AUTOSAVE = 164;
const int IDC_RADIO_BY_MINUTES = 165;
const int IDC_EDIT_MINUTES = 166;
const int IDC_SPIN_MINUTES = 167;
const int IDC_RADIO_BY_MOVES = 168;
const int IDC_EDIT_MOVES = 169;
const int IDC_SPIN_MOVES = 170;
const int IDC_EDIT_KB_MINUTES = 171;
const int IDC_SPIN_KB_MINUTES = 172;
wxSizer *AutoSavingPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_RADIO_DIV1 = 173;
const int IDC_RADIO_DIV2 = 174;
const int IDC_RADIO_DIV3 = 175;
const int IDC_RADIO_DIV4 = 176;
const int IDC_RADIO_DIV5 = 177;
const int IDC_COMBO_CHOOSE_BOOK = 178;
const int ID_TEXT_AS_STATIC = 179;
wxSizer *WhichBookDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_STATIC_MSG = 180;
const int IDC_EDIT_ERR_XML = 181;
const int IDC_STATIC_LBL = 182;
const int IDC_EDIT_OFFSET = 183;
wxSizer *XMLErrorDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_ED_SILCONVERTER_NAME = 184;
const int IDC_BTN_SELECT_SILCONVERTER = 185;
const int IDC_ED_SILCONVERTER_INFO = 186;
const int IDC_BTN_CLEAR_SILCONVERTER = 187;
wxSizer *SilConvertersDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_STATIC_FOLDER_DOCS = 188;
const int IDC_LIST_FOLDER_DOCS = 189;
const int IDC_BUTTON_NEXT_CHAPTER = 190;
const int IDC_BUTTON_SPLIT_NOW = 191;
const int IDC_STATIC_SPLITTING_WAIT = 192;
const int IDC_RADIO_PHRASEBOX_LOCATION = 193;
const int IDC_RADIO_CHAPTER_SFMARKER = 194;
const int IDC_RADIO_DIVIDE_INTO_CHAPTERS = 195;
const int IDC_STATIC_SPLIT_NAME = 196;
const int IDC_EDIT1 = 197;
const int IDC_STATIC_REMAIN_NAME = 198;
const int IDC_EDIT2 = 199;
wxSizer *SplitDialogFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_LIST_MARKERS = 200;
const int IDC_TEXTCTRL_AS_STATIC_PLACE_INT_MKRS = 201;
wxSizer *PlaceInternalMarkersDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_NOTE = 202;
const int IDC_EDIT_FIND_TEXT = 203;
const int IDC_FIND_NEXT_BTN = 204;
const int IDC_LAST_BTN = 205;
const int IDC_NEXT_BTN = 206;
const int IDC_PREV_BTN = 207;
const int IDC_FIRST_BTN = 208;
const int IDC_DELETE_BTN = 209;
wxSizer *NoteDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_AS_STATIC_COLLECT_BT = 210;
const int IDC_RADIO_COLLECT_ADAPTATIONS = 211;
const int IDC_RADIO_COLLECT_GLOSSES = 212;
wxSizer *CollectBackTranslationsDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_SPIN_DELAY_TICKS = 213;
wxSizer *SetDelayDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_RADIO_EXPORT_ALL = 214;
const int IDC_RADIO_EXPORT_SELECTED_MARKERS = 215;
const int ID_TEXTCTRL_AS_STATIC_EXPORT_OPTIONS = 216;
const int IDC_LIST_SFMS = 217;
const int IDC_BUTTON_EXCLUDE_FROM_EXPORT = 218;
const int IDC_BUTTON_INCLUDE_IN_EXPORT = 219;
const int IDC_BUTTON_UNDO = 220;
const int IDC_STATIC_SPECIAL_NOTE = 221;
const int IDC_CHECK_PLACE_FREE_TRANS = 222;
const int IDC_CHECK_INTERLINEAR_PLACE_FREE_TRANS = 223;
const int IDC_CHECK_PLACE_BACK_TRANS = 224;
const int IDC_CHECK_INTERLINEAR_PLACE_BACK_TRANS = 225;
const int IDC_CHECK_PLACE_AI_NOTES = 226;
const int IDC_CHECK_INTERLINEAR_PLACE_AI_NOTES = 227;
wxSizer *ExportOptionsDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_STATIC_TITLE = 228;
const int IDC_RADIO_EXPORT_AS_SFM = 229;
const int ID_TEXTCTRL_AS_STATIC_EXPORT_SAVE_AS_1 = 230;
const int IDC_RADIO_EXPORT_AS_RTF = 231;
const int ID_TEXTCTRL_AS_STATIC_EXPORT_SAVE_AS_2 = 232;
const int ID_TEXTCTRL_AS_STATIC_EXPORT_SAVE_AS_3 = 233;
const int IDC_BUTTON_EXPORT_FILTER_OPTIONS = 234;
wxSizer *ExportSaveAsDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_CHECK_INCLUDE_NAV_TEXT = 235;
const int IDC_CHECK_INCLUDE_SOURCE_TEXT = 236;
const int IDC_CHECK_INCLUDE_TARGET_TEXT = 237;
const int IDC_CHECK_INCLUDE_GLS_TEXT = 238;
const int ID_TEXTCTRL_AS_STATIC_EXP_INTERLINEAR = 239;
const int IDC_RADIO_PORTRAIT = 240;
const int IDC_RADIO_LANDSCAPE = 241;
const int IDC_RADIO_OUTPUT_ALL = 242;
const int IDC_RADIO_OUTPUT_CHAPTER_VERSE_RANGE = 243;
const int IDC_EDIT_FROM_CHAPTER = 244;
const int IDC_EDIT_FROM_VERSE = 245;
const int IDC_EDIT_TO_CHAPTER = 246;
const int IDC_EDIT_TO_VERSE = 247;
const int IDC_RADIO_OUTPUT_PRELIM = 248;
const int IDC_RADIO_OUTPUT_FINAL = 249;
const int ID_CHECK_NEW_TABLES_FOR_NEWLINE_MARKERS = 250;
const int ID_CHECK_CENTER_TABLES_FOR_CENTERED_MARKERS = 251;
const int IDC_BUTTON_RTF_EXPORT_FILTER_OPTIONS = 252;
wxSizer *ExportInterlinearDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_LIST_MARKER = 253;
const int IDC_EDIT_MARKER_TEXT = 254;
const int IDC_REMOVE_BTN = 255;
const int IDC_BUTTON_SWITCH_ENCODING = 256;
const int IDC_LIST_MARKER_END = 257;
const int IDC_STATIC_MARKER_DESCRIPTION = 258;
const int IDC_STATIC_MARKER_STATUS = 259;
wxSizer *ViewFilteredMaterialDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_CHAPTER = 260;
const int IDC_EDIT_VERSE = 261;
wxSizer *GoToDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_KEY = 262;
const int IDC_EDIT_CH_VERSE = 263;
const int IDC_EDIT_ADAPTATION = 264;
const int IDC_LIST_TRANSLATIONS = 265;
const int IDC_RADIO_LIST_SELECT = 266;
const int IDC_RADIO_ACCEPT_CURRENT = 267;
const int IDC_RADIO_TYPE_NEW = 268;
const int IDC_EDIT_TYPE_NEW = 269;
const int IDC_NOTHING = 270;
const int IDC_BUTTON_IGNORE_IT = 271;
const int IDC_CHECK_DO_SAME = 272;
wxSizer *ConsistencyCheckDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_RADIO_CHECK_OPEN_DOC_ONLY = 273;
const int IDC_RADIO_CHECK_SELECTED_DOCS = 274;
const int ID_TEXTCTRL_AS_STATIC_CHOOSE_CONSISTENCY_CHECK_TYPE = 275;
const int ID_TEXTCTRL_MSG_TWO = 276;
wxSizer *ChooseConsistencyCheckTypeDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_EDIT_SOURCE_AS_STATIC0 = 277;
const int ID_TEXTCTRL = 278;
const int IDC_EDIT_OLD_SOURCE_TEXT = 279;
const int ID_TEXTCTRL_EDIT_SOURCE_AS_STATIC1 = 280;
const int ID_TEXTCTRL_EDIT_SOURCE_AS_STATIC2 = 281;
const int IDC_EDIT_NEW_SOURCE = 282;
const int ID_TEXTCTRL_EDIT_SOURCE_AS_STATIC4 = 283;
wxSizer *EditSourceTextDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_UNPACK_WARN = 284;
wxSizer *UnpackWarningDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_STATIC_COUNT = 285;
const int IDC_LIST_SRC_KEYS = 286;
const int IDC_LIST_EXISTING_TRANSLATIONS = 287;
const int IDC_EDIT_EDITORADD = 288;
const int IDC_BUTTON_UPDATE = 289;
const int IDC_BUTTON_ADD = 290;
const int IDC_ADD_NOTHING = 291;
const int IDC_EDIT_SHOW_FLAG = 292;
const int IDC_BUTTON_FLAG_TOGGLE = 293;
const int IDC_EDIT_SRC_KEY = 294;
const int IDC_EDIT_REF_COUNT = 295;
const int ID_TEXTCTRL_SEARCH = 296;
const int ID_BUTTON_GO = 297;
const int ID_BUTTON_ERASE_ALL_LINES = 298;
const int ID_COMBO_OLD_SEARCHES = 299;
wxSizer *KBEditorPanelFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_STATIC_TEXT_SELECT_A_TAB = 300;
const int ID_STATIC_WHICH_KB = 301;
const int ID_KB_EDITOR_NOTEBOOK = 302;
wxSizer *KBEditorDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_STATIC_DOCS_IN_FOLDER = 303;
const int IDC_LIST_SOURCE_FOLDER_DOCS = 304;
const int IDC_RADIO_TO_BOOK_FOLDER = 305;
const int IDC_RADIO_FROM_BOOK_FOLDER = 306;
const int IDC_VIEW_OTHER = 307;
const int IDC_BUTTON_RENAME_DOC = 308;
const int IDC_MOVE_NOW = 309;
wxSizer *MoveDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_AS_STATIC_JOIN1 = 310;
const int IDC_BUTTON_MOVE_ALL_RIGHT = 311;
const int IDC_BUTTON_MOVE_ALL_LEFT = 312;
const int ID_TEXTCTRL_AS_STATIC_JOIN2 = 313;
const int ID_TEXTCTRL_AS_STATIC_JOIN3 = 314;
const int ID_JOIN_NOW = 315;
const int IDC_STATIC_JOINING_WAIT = 316;
const int ID_TEXTCTRL_AS_STATIC_JOIN4 = 317;
const int IDC_EDIT_NEW_FILENAME = 318;
wxSizer *JoinDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

wxSizer *ListDocInOtherFolderDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_STATIC_SET_FONT_TITLE = 319;
const int ID_STATIC_CURR_ENCODING_IS = 320;
const int ID_TEXT_CURR_ENCODING = 321;
const int ID_LISTBOX_POSSIBLE_FACENAMES = 322;
const int ID_LISTBOX_POSSIBLE_ENCODINGS = 323;
extern wxSizer *pTestBoxStaticTextBoxSizer;
const int ID_TEXT_TEST_ENCODING_BOX = 324;
const int ID_BUTTON_CHART_FONT_SIZE_DECREASE = 325;
const int ID_BUTTON_CHART_FONT_SIZE_INCREASE = 326;
const int ID_STATIC_CHART_FONT_SIZE = 327;
const int ID_SCROLLED_ENCODING_WINDOW = 328;
wxSizer *SetEncodingDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_CCT = 329;
wxSizer *CCTableEditDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_LIST_CCTABLES = 330;
const int IDC_EDIT_SELECTED_TABLE = 331;
const int IDC_BUTTON_BROWSE = 332;
const int IDC_BUTTON_CREATE_CCT = 333;
const int IDC_BUTTON_EDIT_CCT = 334;
const int IDC_BUTTON_SELECT_NONE = 335;
const int IDC_EDIT_FOLDER_PATH = 336;
wxSizer *CCTablePageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_RADIO_SRC_ONLY_FIND = 337;
const int IDC_RADIO_TGT_ONLY_FIND = 338;
const int IDC_RADIO_SRC_AND_TGT_FIND = 339;
const int IDC_CHECK_IGNORE_CASE_FIND = 340;
const int IDC_STATIC_SRC_FIND = 341;
const int IDC_EDIT_SRC_FIND = 342;
const int IDC_STATIC_TGT_FIND = 343;
const int IDC_EDIT_TGT_FIND = 344;
const int IDC_CHECK_INCLUDE_PUNCT_FIND = 345;
const int IDC_CHECK_SPAN_SRC_PHRASES_FIND = 346;
const int IDC_STATIC_SPECIAL = 347;
extern wxSizer *IDC_STATIC_SIZER_SPECIAL;
const int IDC_RADIO_RETRANSLATION = 348;
const int IDC_RADIO_NULL_SRC_PHRASE = 349;
const int IDC_RADIO_SFM = 350;
const int IDC_STATIC_SELECT_MKR = 351;
const int IDC_COMBO_SFM = 352;
const int IDC_BUTTON_SPECIAL = 353;
wxSizer *FindDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_RADIO_SRC_ONLY_REPLACE = 354;
const int IDC_RADIO_TGT_ONLY_REPLACE = 355;
const int IDC_RADIO_SRC_AND_TGT_REPLACE = 356;
const int IDC_CHECK_IGNORE_CASE_REPLACE = 357;
const int IDC_STATIC_SRC_REPLACE = 358;
const int IDC_EDIT_SRC_REPLACE = 359;
const int IDC_STATIC_TGT_REPLACE = 360;
const int IDC_EDIT_TGT_REPLACE = 361;
const int IDC_CHECK_INCLUDE_PUNCT_REPLACE = 362;
const int IDC_CHECK_SPAN_SRC_PHRASES_REPLACE = 363;
const int IDC_STATIC_REPLACE = 364;
const int IDC_EDIT_REPLACE = 365;
const int IDC_REPLACE = 366;
const int IDC_REPLACE_ALL_BUTTON = 367;
wxSizer *ReplaceDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_AS_STATIC = 368;
const int IDC_EDIT_TBLNAME = 369;
wxSizer *CCTableNameDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_EDIT_AS_STATIC = 370;
const int ID_RADIO_ALL = 371;
const int ID_RADIO_SELECTION = 372;
const int IDC_RADIO_PAGES = 373;
const int IDC_EDIT_PAGES_FROM = 374;
const int IDC_EDIT_PAGES_TO = 375;
const int IDC_RADIO_CHAPTER_VERSE_RANGE = 376;
const int IDC_EDIT3 = 377;
const int IDC_EDIT4 = 378;
const int IDC_CHECK_SUPPRESS_PREC_HEADING = 379;
const int IDC_CHECK_INCLUDE_FOLL_HEADING = 380;
const int IDC_CHECK_SUPPRESS_FOOTER = 381;
wxSizer *PrintOptionsDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_CC_TABBED_NOTEBOOK = 382;
wxSizer *CCTabbedNotebookFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_LIST_UI_LANGUAGES = 383;
const int ID_TEXT_AS_STATIC_SHORT_LANG_NAME = 384;
const int ID_TEXT_AS_STATIC_LONG_LANG_NAME = 385;
const int IDC_LOCALIZATION_PATH = 386;
const int IDC_BTN_BROWSE_PATH = 387;
wxSizer *ChooseLanguageDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_COMBO_REMOVALS = 388;
const int ID_STATIC_TEXT_REMOVALS = 389;
const int IDC_BUTTON_UNDO_LAST_COPY = 390;
wxSizer *RemovalsBarFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_MSG_TEXT = 391;
const int IDC_BUTTON_PREV_STEP = 392;
const int IDC_BUTTON_NEXT_STEP = 393;
const int ID_BUTTON_END_NOW = 394;
const int ID_BUTTON_CANCEL_ALL_STEPS = 395;
wxSizer *VertEditBarFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXT_TITLE = 396;
const int ID_TEXTCTRL_AS_STATIC_PUNCT_CORRESP_PAGE = 397;
const int IDC_TOGGLE_UNNNN_BTN = 398;
const int ID_NOTEBOOK = 399;
wxSizer *PunctCorrespPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_SRC0 = 400;
const int IDC_EDIT_TGT0 = 401;
const int IDC_EDIT_SRC9 = 402;
const int IDC_EDIT_TGT9 = 403;
const int IDC_EDIT_SRC18 = 404;
const int IDC_EDIT_TGT18 = 405;
const int IDC_EDIT_SRC1 = 406;
const int IDC_EDIT_TGT1 = 407;
const int IDC_EDIT_SRC10 = 408;
const int IDC_EDIT_TGT10 = 409;
const int IDC_EDIT_SRC19 = 410;
const int IDC_EDIT_TGT19 = 411;
const int IDC_EDIT_SRC2 = 412;
const int IDC_EDIT_TGT2 = 413;
const int IDC_EDIT_SRC11 = 414;
const int IDC_EDIT_TGT11 = 415;
const int IDC_EDIT_SRC20 = 416;
const int IDC_EDIT_TGT20 = 417;
const int IDC_EDIT_SRC3 = 418;
const int IDC_EDIT_TGT3 = 419;
const int IDC_EDIT_SRC12 = 420;
const int IDC_EDIT_TGT12 = 421;
const int IDC_EDIT_SRC21 = 422;
const int IDC_EDIT_TGT21 = 423;
const int IDC_EDIT_SRC4 = 424;
const int IDC_EDIT_TGT4 = 425;
const int IDC_EDIT_SRC13 = 426;
const int IDC_EDIT_TGT13 = 427;
const int IDC_EDIT_SRC22 = 428;
const int IDC_EDIT_TGT22 = 429;
const int IDC_EDIT_SRC5 = 430;
const int IDC_EDIT_TGT5 = 431;
const int IDC_EDIT_SRC14 = 432;
const int IDC_EDIT_TGT14 = 433;
const int IDC_EDIT_SRC23 = 434;
const int IDC_EDIT_TGT23 = 435;
const int IDC_EDIT_SRC6 = 436;
const int IDC_EDIT_TGT6 = 437;
const int IDC_EDIT_SRC15 = 438;
const int IDC_EDIT_TGT15 = 439;
const int IDC_EDIT_SRC24 = 440;
const int IDC_EDIT_TGT24 = 441;
const int IDC_EDIT_SRC7 = 442;
const int IDC_EDIT_TGT7 = 443;
const int IDC_EDIT_SRC16 = 444;
const int IDC_EDIT_TGT16 = 445;
const int IDC_EDIT_SRC25 = 446;
const int IDC_EDIT_TGT25 = 447;
const int IDC_EDIT_SRC8 = 448;
const int IDC_EDIT_TGT8 = 449;
const int IDC_EDIT_SRC17 = 450;
const int IDC_EDIT_TGT17 = 451;
wxSizer *SinglePunctTabPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_EDIT_2SRC0 = 452;
const int IDC_EDIT_2TGT0 = 453;
const int IDC_EDIT_2SRC5 = 454;
const int IDC_EDIT_2TGT5 = 455;
const int IDC_EDIT_2SRC1 = 456;
const int IDC_EDIT_2TGT1 = 457;
const int IDC_EDIT_2SRC6 = 458;
const int IDC_EDIT_2TGT6 = 459;
const int IDC_EDIT_2SRC2 = 460;
const int IDC_EDIT_2TGT2 = 461;
const int IDC_EDIT_2SRC7 = 462;
const int IDC_EDIT_2TGT7 = 463;
const int IDC_EDIT_2SRC3 = 464;
const int IDC_EDIT_2TGT3 = 465;
const int IDC_EDIT_2SRC8 = 466;
const int IDC_EDIT_2TGT8 = 467;
const int IDC_EDIT_2SRC4 = 468;
const int IDC_EDIT_2TGT4 = 469;
const int IDC_EDIT_2SRC9 = 470;
const int IDC_EDIT_2TGT9 = 471;
wxSizer *DoublePunctTabPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

extern wxSizer *ID_CONTROLBAR_2_LINE_SIZER_TOP;
extern wxSizer *ID_CONTROLBAR_2_LINE_SIZER_BOTTOM;
wxSizer *ControlBar2LineFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_MSG1 = 472;
const int ID_BUTTON_LOCATE_SOURCE_FOLDER = 473;
const int ID_BITMAPBUTTON_SRC_OPEN_FOLDER_UP = 474;
const int ID_TEXT_SOURCE_FOLDER_PATH = 475;
const int ID_TEXTCTRL_SOURCE_PATH = 476;
const int ID_LISTCTRL_SOURCE_CONTENTS = 477;
const int ID_BUTTON_LOCATE_DESTINATION_FOLDER = 478;
const int ID_BITMAPBUTTON_DEST_OPEN_FOLDER_UP = 479;
const int ID_TEXTCTRL_DESTINATION_PATH = 480;
const int ID_LISTCTRL_DESTINATION_CONTENTS = 481;
const int ID_BUTTON_MOVE = 482;
const int ID_BUTTON_COPY = 483;
const int ID_BUTTON_PEEK = 484;
const int ID_BUTTON_RENAME = 485;
const int ID_BUTTON_DELETE = 486;
wxSizer *MoveOrCopyFilesOrFoldersFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXT_MSG1 = 487;
const int ID_TEXTCTRL_SOURCE_FILE_DETAILS = 488;
const int ID_TEXTCTRL_DESTINATION_FILE_DETAILS = 489;
const int ID_RADIOBUTTON_REPLACE = 490;
const int ID_RADIOBUTTON_NO_COPY = 491;
const int ID_RADIOBUTTON_COPY_AND_RENAME = 492;
const int ID_TEXT_MODIFY_NAME = 493;
const int ID_CHECKBOX_HANDLE_SAME = 494;
wxSizer *FilenameConflictFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_LISTBOX_UPDATED = 495;
const int ID_TEXTCTRL_INFO_SOURCE = 496;
const int ID_TEXTCTRL_INFO_REFS = 497;
const int ID_LISTBOX_MATCHED = 498;
const int ID_BUTTON_UPDATE = 499;
const int ID_TEXTCTRL_LOCAL_SEARCH = 500;
const int ID_BUTTON_FIND_NEXT = 501;
const int ID_TEXTCTRL_EDITBOX = 502;
const int ID_BUTTON_RESTORE = 503;
const int ID_BUTTON_REMOVE_UPDATE = 504;
wxSizer *KBEditSearchFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_STATICTEXT_SCROLL_LIST = 505;
const int ID_LIST_LANGUAGE_CODES_NAMES = 506;
const int ID_STATICTEXT_SEARCH_FOR_LANG_NAME = 507;
const int ID_TEXTCTRL_SEARCH_LANG_NAME = 508;
const int ID_BUTTON_USE_SEL_AS_SRC = 509;
const int ID_BUTTON_USE_SEL_AS_TGT = 510;
const int ID_SRC_LANGUAGE_CODE = 511;
const int ID_TEXTCTRL_SRC_LANG_CODE = 512;
const int ID_TGT_LANGUAGE_CODE = 513;
const int ID_TEXTCTRL_TGT_LANG_CODE = 514;
wxSizer *LanguageCodesDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_PEEKMSG = 515;
extern wxSizer *m_pHorizBox_for_textctrl;
const int ID_TEXTCTRL_LINES100 = 516;
const int ID_BUTTON_TOGGLE_TEXT_DIRECTION = 517;
wxSizer *PeekAtFileFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_INSTRUCTIONS = 518;
const int ID_LISTBOX_LOADABLES_FILENAMES = 519;
wxSizer *NewDocFromSourceDataFolderFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_STATIC_SELECT_A_TAB = 520;
const int ID_MENU_EDITOR_NOTEBOOK = 521;
const int ID_RADIOBUTTON_NONE = 522;
const int ID_RADIOBUTTON_USE_PROFILE = 523;
const int ID_COMBO_PROFILE_ITEMS = 524;
const int ID_BUTTON_RESET_TO_FACTORY = 525;
const int ID_TEXT_STATIC_DESCRIPTION = 526;
const int ID_TEXTCTRL_PROFILE_DESCRIPTION = 527;
wxSizer *MenuEditorDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_CHECKLISTBOX_MENU_ITEMS = 528;
wxSizer *MenuEditorPanelFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int IDC_RADIO_USE_UBS_SET_ONLY = 529;
const int IDC_RADIO_USE_SILPNG_SET_ONLY = 530;
const int IDC_RADIO_USE_BOTH_SETS = 531;
const int IDC_RADIO_USE_UBS_SET_ONLY_PROJ = 532;
const int IDC_RADIO_USE_SILPNG_SET_ONLY_PROJ = 533;
const int IDC_RADIO_USE_BOTH_SETS_PROJ = 534;
const int IDC_CHECK_CHANGE_FIXED_SPACES_TO_REGULAR_SPACES_USFM = 535;
const int ID_TEXTCTRL_AS_STATIC_FILTERPAGE = 536;
const int IDC_LIST_SFMS_PROJ = 537;
wxSizer *UsfmFilterPageFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_CHECK_USE_GUESSER = 538;
const int ID_PANEL_GUESS_COLOR_DISPLAY = 539;
const int ID_BUTTON_GUESS_HIGHLIGHT_COLOR = 540;
const int ID_SLIDER_GUESSER = 541;
const int ID_CHECK_ALLOW_GUESSER_ON_UNCHANGED_CC_OUTPUT = 542;
const int ID_TEXT_STATIC_NUM_CORRESP_ADAPTATIONS_GUESSER = 543;
const int ID_TEXT_STATIC_NUM_CORRESP_GLOSSING_GUESSER = 544;
wxSizer *GuesserSettingsDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

const int ID_TEXTCTRL_DEVS_EMAIL_ADDR = 545;
const int ID_TEXTCTRL_MY_EMAIL_ADDR = 546;
const int ID_TEXTCTRL_SUMMARY_SUBJECT = 547;
extern wxSizer *STATIC_TEXT_DESCRIPTION;
const int ID_TEXTCTRL_DESCRIPTION_BODY = 548;
const int ID_TEXTCTRL_SENDERS_NAME = 549;
const int ID_CHECKBOX_LET_DEVS_KNOW_AI_USAGE = 550;
const int ID_BUTTON_VIEW_USAGE_LOG = 551;
const int ID_TEXT_AI_VERSION = 552;
const int ID_TEXT_RELEASE_DATE = 553;
const int ID_TEXT_DATA_TYPE = 554;
const int ID_TEXT_FREE_MEMORY = 555;
const int ID_TEXT_SYS_LOCALE = 556;
const int ID_TEXT_INTERFACE_LANGUAGE = 557;
const int ID_TEXT_SYS_ENCODING = 558;
const int ID_TEXT_SYS_LAYOUT_DIR = 559;
const int ID_TEXT_WXWIDGETS_VERSION = 560;
const int ID_TEXT_OS_VERSION = 561;
const int ID_BUTTON_SAVE_REPORT_AS_TEXT_FILE = 562;
const int ID_BUTTON_LOAD_SAVED_REPORT = 563;
const int ID_RADIOBUTTON_SEND_DIRECTLY_FROM_AI = 564;
const int ID_RADIOBUTTON_SEND_TO_MY_EMAIL = 565;
const int ID_BUTTON_ATTACH_PACKED_DOC = 566;
const int ID_BUTTON_SEND_NOW = 567;
wxSizer *EmailReportDlgFunc( wxWindow *parent, bool call_fit = TRUE, bool set_sizer = TRUE );

// Declare menubar functions

const int ID_FILE_MENU = 568;
const int ID_SAVE_AS = 569;
const int ID_FILE_PACK_DOC = 570;
const int ID_FILE_UNPACK_DOC = 571;
const int ID_MENU = 572;
const int ID_FILE_STARTUP_WIZARD = 573;
const int ID_FILE_CLOSEKB = 574;
const int ID_FILE_CHANGEFOLDER = 575;
const int ID_FILE_SAVEKB = 576;
const int ID_FILE_BACKUP_KB = 577;
const int ID_FILE_RESTORE_KB = 578;
const int ID_EDIT_MENU = 579;
const int ID_EDIT_CUT = 580;
const int ID_EDIT_COPY = 581;
const int ID_EDIT_PASTE = 582;
const int ID_GO_TO = 583;
const int ID_EDIT_SOURCE_TEXT = 584;
const int ID_EDIT_CONSISTENCY_CHECK = 585;
const int ID_EDIT_MOVE_NOTE_FORWARD = 586;
const int ID_EDIT_MOVE_NOTE_BACKWARD = 587;
const int ID_VIEW_MENU = 588;
const int ID_VIEW_TOOLBAR = 589;
const int ID_VIEW_STATUS_BAR = 590;
const int ID_VIEW_COMPOSE_BAR = 591;
const int ID_COPY_SOURCE = 592;
const int ID_MARKER_WRAPS_STRIP = 593;
const int ID_UNITS = 594;
const int ID_CHANGE_INTERFACE_LANGUAGE = 595;
const int ID_TOOLS_MENU = 596;
const int ID_TOOLS_DEFINE_CC = 597;
const int ID_UNLOAD_CC_TABLES = 598;
const int ID_USE_CC = 599;
const int ID_ACCEPT_CHANGES = 600;
const int ID_TOOLS_DEFINE_SILCONVERTER = 601;
const int ID_USE_SILCONVERTER = 602;
const int ID_TOOLS_KB_EDITOR = 603;
const int ID_TOOLS_AUTO_CAPITALIZATION = 604;
const int ID_RETRANS_REPORT = 605;
const int ID_TOOLS_SPLIT_DOC = 606;
const int ID_TOOLS_JOIN_DOCS = 607;
const int ID_TOOLS_MOVE_DOC = 608;
const int ID_EXPORT_IMPORT_MENU = 609;
const int ID_FILE_EXPORT_SOURCE = 610;
const int ID_FILE_EXPORT = 611;
const int ID_FILE_EXPORT_TO_RTF = 612;
const int ID_EXPORT_GLOSSES = 613;
const int ID_EXPORT_FREE_TRANS = 614;
const int ID_EXPORT_OXES = 615;
const int ID_FILE_EXPORT_KB = 616;
const int ID_IMPORT_TO_KB = 617;
const int ID_ADVANCED_MENU = 618;
const int ID_ADVANCED_ENABLEGLOSSING = 619;
const int ID_ADVANCED_GLOSSING_USES_NAV_FONT = 620;
const int ID_ADVANCED_TRANSFORM_ADAPTATIONS_INTO_GLOSSES = 621;
const int ID_ADVANCED_DELAY = 622;
const int ID_ADVANCED_BOOKMODE = 623;
const int ID_ADVANCED_FREE_TRANSLATION_MODE = 624;
const int ID_ADVANCED_TARGET_TEXT_IS_DEFAULT = 625;
const int ID_ADVANCED_GLOSS_TEXT_IS_DEFAULT = 626;
const int ID_ADVANCED_REMOVE_FILTERED_FREE_TRANSLATIONS = 627;
const int ID_ADVANCED_COLLECT_BACKTRANSLATIONS = 628;
const int ID_ADVANCED_REMOVE_FILTERED_BACKTRANSLATIONS = 629;
const int ID_ADVANCED_USETRANSLITERATIONMODE = 630;
const int ID_ADVANCED_SENDSYNCHRONIZEDSCROLLINGMESSAGES = 631;
const int ID_ADVANCED_RECEIVESYNCHRONIZEDSCROLLINGMESSAGES = 632;
const int ID_LAYOUT_MENU = 633;
const int ID_ALIGNMENT = 634;
const int ID_HELP_MENU = 635;
const int ID_ONLINE_HELP = 636;
const int ID_REPORT_A_PROBLEM = 637;
const int ID_GIVE_FEEDBACK = 638;
const int ID_HELP_USE_TOOLTIPS = 639;
const int ID_ADMINISTRATOR_MENU = 640;
const int ID_CUSTOM_WORK_FOLDER_LOCATION = 641;
const int ID_LOCK_CUSTOM_LOCATION = 642;
const int ID_UNLOCK_CUSTOM_LOCATION = 643;
const int ID_LOCAL_WORK_FOLDER_MENU = 644;
const int ID_SET_PASSWORD_MENU = 645;
const int ID_SETUP_PARATEXT_COLLABORATION = 646;
const int ID_MOVE_OR_COPY_FOLDERS_OR_FILES = 647;
const int ID_SOURCE_DATA_FOLDER = 648;
const int ID_EXPORT_DATA_FOLDER = 649;
const int ID_EDIT_USER_MENU_SETTINGS_PROFILE = 650;
wxMenuBar *AIMenuBarFunc();

// Declare toolbar functions

const int ID_BUTTON_GUESSER = 651;
const int ID_BUTTON_CREATE_NOTE = 652;
const int ID_BUTTON_PREV_NOTE = 653;
const int ID_BUTTON_NEXT_NOTE = 654;
const int ID_BUTTON_DELETE_ALL_NOTES = 655;
const int ID_BUTTON_RESPECTING_BDRY = 656;
const int ID_BUTTON_SHOWING_PUNCT = 657;
const int ID_BUTTON_TO_END = 658;
const int ID_BUTTON_TO_START = 659;
const int ID_BUTTON_STEP_DOWN = 660;
const int ID_BUTTON_STEP_UP = 661;
const int ID_BUTTON_BACK = 662;
const int ID_BUTTON_MERGE = 663;
const int ID_BUTTON_RETRANSLATION = 664;
const int ID_BUTTON_EDIT_RETRANSLATION = 665;
const int ID_REMOVE_RETRANSLATION = 666;
const int ID_BUTTON_NULL_SRC = 667;
const int ID_BUTTON_REMOVE_NULL_SRCPHRASE = 668;
const int ID_BUTTON_CHOOSE_TRANSLATION = 669;
const int ID_SHOWING_ALL = 670;
const int ID_BUTTON_EARLIER_TRANSLATION = 671;
const int ID_BUTTON_NO_PUNCT_COPY = 672;
void AIToolBarFunc( wxToolBar *parent );

void AIToolBar32x30Func( wxToolBar *parent );

// Declare bitmap functions

wxBitmap AIToolBarBitmapsUnToggledFunc( size_t index );

const int ID_BITMAP_FOLDERAI = 673;
const int ID_BITMAP_FILEAI = 674;
const int ID_BITMAP_EMPTY_FOLDER = 675;
wxBitmap AIMainFrameIcons( size_t index );

const int ID_BUTTON_IGNORING_BDRY = 676;
const int ID_BUTTON_HIDING_PUNCT = 677;
const int ID_SHOWING_TGT = 678;
const int ID_BUTTON_ENABLE_PUNCT_COPY = 679;
wxBitmap AIToolBarBitmapsToggledFunc( size_t index );

wxBitmap WhichFilesBitmapsFunc( size_t index );

wxBitmap AIToolBarBitmapsToggled32x30Func( size_t index );

wxBitmap AIToolBarBitmapsUnToggled32x30Func( size_t index );

#endif

// End of generated file
