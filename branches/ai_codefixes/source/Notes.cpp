...
/// \date_revised	16 Februuary 2010
...
/////////////////////////////////////////////////////////////////////////////////
bool CNotes::GetMovedNotesSpan(SPList* pSrcPhrases, EditRecord* pRec, 
...
void CNotes::JumpBackwardToNote_CoreCode(int nJumpOffSequNum)
{
	...
	if (nNoteSequNum > 0)
	{
		...
		else if (pSrcPhrase->m_bRetranslation && !gbIsGlossing)
		{
            ...
            // then before it
			CSourcePhrase* pSafeSrcPhrase = GetView()->GetFollSafeSrcPhrase(pSrcPhrase);
			if (pSafeSrcPhrase == NULL)
			{
				// not a safe place, so try earlier than the retranslation
				pSafeSrcPhrase = GetView()->GetPrevSafeSrcPhrase(pSrcPhrase);
				if (pSafeSrcPhrase == NULL)
				{
...
else
{
	...
	GetView()->OnButtonEnablePunctCopy(event);
	GetView()->Invalidate();
	GetLayout()->PlaceBox();
...
void CNotes::JumpForwardToNote_CoreCode(int nJumpOffSequNum)
{
	...
	if (nNoteSequNum <= m_pApp->GetMaxIndex())
	{
		...
		else if (pSrcPhrase->m_bRetranslation && !gbIsGlossing)
		{
            ...
            // then after it
			CSourcePhrase* pSafeSrcPhrase = GetView()->GetPrevSafeSrcPhrase(pSrcPhrase);
			if (pSafeSrcPhrase == NULL)
			{
				// not a safe place, so try after the retranslation
				pSafeSrcPhrase = GetView()->GetFollSafeSrcPhrase(pSrcPhrase);
				if (pSafeSrcPhrase == NULL)
				{
...
/////////////////////////////////////////////////////////////////////////////////
bool CNotes::MoveNoteLocationsLeftwardsOnce(wxArrayInt* pLocationsList, 
											int nLeftBoundSN)
{
	// BEW added 30May08 in support of the source text editing step of the 
	// vertical editing process
...
/////////////////////////////////////////////////////////////////////////////////
bool CNotes::ShiftASeriesOfConsecutiveNotesRightwardsOnce(SPList* pSrcPhrases, 
																 int nFirstNoteSN)
{
	// refactored 7Apr09 - only needed GetMaxIndex() call
...
void CNotes::OnButtonCreateNote(wxCommandEvent& WXUNUSED(event))
{
	...
	if (m_pApp->m_selectionLine != -1)
	{
		...
		wxASSERT(pPile != NULL);
		GetView()->RemoveSelection(); // need Invalidate() to be called later on
	}
	else
	{
