...
#               Third parameter -notag is optional and will bypass downloac/checkout
#                 of a specific tagged version from GitHub to build the packages, that
#                 is, the packages will be built from the current state of the code 
#                 residing in ~/projects/adaptit (no git pull updates performed).
#
...
# Revised 2021-02-6 by Bill Martin
#	- Added a -notag option as an optional third parameter at command line. Using -notag will 
#     bypass the downloac/checkout of a specific tagged version from GitHub before building the 
#     packages. With -notag the packages will be built from the current state of the code 
#     residing in ~/projects/adaptit (no git pull updates performed before packages are built).
...
#  echo "bsOld = $bsOld"
  let "buildSuffix=buildSuffix+1"
  echo "Incrementing buildSuffix from $bsOld to $buildSuffix, use editor to finalize changelog edit."
  dch --newversion $1-$buildSuffix
fi
...
# TODO: Add more --exclude things when -notag option is being used, since there are many other non tracked things in our working copy
...
# -sa Passed unchanged to dpkg-genchanges where it Forces the inclusion of the original source.
